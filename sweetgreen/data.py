...
def clean_ingredients_web_scrape(file_path=FILE_PATH_CLEAN, file_name="cleaned_web_scraped_ingredients.json"):
...
def compile_ingredients_json(raw_file_path=RAW_FILE_PATH,
    ...
    restaurant_jsons = utils.load_all_json_directory(os.path.join(raw_file_path, restaurant_menu_directory))

    compiled_ingredients = {}
    for restaurant_json in restaurant_jsons:
        ingredients_jsons = restaurant_json["ingredients"]

        compiled_ingredients = utils.compile_jsons(ingredients_jsons, "id", compiled_ingredients)
